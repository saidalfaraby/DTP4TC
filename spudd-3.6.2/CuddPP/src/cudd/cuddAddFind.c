...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddAddFind.c,v 2.0 2003/02/07 00:12:30 staubin Exp $";
#endif
  ...
  int  bit)
{
    ...
    */
    ...
    bitTerminal.set((double) bit);
    index = cuddUniqueConst(dd,&bitTerminal);
  ...
  DdNode * index)
{
    ...
    /* Check terminal case. */
    if (cuddIsConstant(f)) {
	mask = 1 << ((int) (*cuddV(index)).get_val());
	value = (int) (*cuddV(f)).get_val();
