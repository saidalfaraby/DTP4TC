...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddInit.c,v 2.0 2003/02/07 00:12:34 staubin Exp $";
#endif
  ...
  unsigned long maxMemory /* target maximum memory occupation */)
{
    ...
    oneTerminal.set(1.0);
    Terminal zeroTerminal;
    zeroTerminal.set(0.0);
    unique->one = cuddUniqueConst(unique,&oneTerminal);
    //unique->one = cuddUniqueConst(unique,1.0);
    if (unique->one == NULL) return(0);
    cuddRef(unique->one);
    unique->zero = cuddUniqueConst(unique,&zeroTerminal);
...
#endif
    Terminal infTerminal;
    infTerminal.set(DD_PLUS_INF_VAL);
    unique->plusinfinity = cuddUniqueConst(unique,&infTerminal);
    ...
    Terminal infminusTerminal;
    infminusTerminal.set(DD_MINUS_INF_VAL);
    unique->minusinfinity = cuddUniqueConst(unique,&infminusTerminal);
