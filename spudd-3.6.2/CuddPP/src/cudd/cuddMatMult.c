...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddMatMult.c,v 2.0 2003/02/07 00:12:36 staubin Exp $";
#endif
  ...
  int * vars)
{
	...
	*/
	...
	Terminal scaleTerminal;
	scaleTerminal.set((double) scale);
	Terminal terminal;
	Terminal terminal1;
	terminal = ((*cuddV(A)) * (*cuddV(B)));
	terminal1 = terminal * scaleTerminal;
	...
	*/
	...
	if (scale > 1) {
	    cuddRef(res);
	    Terminal scaleTerminal1;
	    scaleTerminal1.set((double) scale);
	    add_scale = cuddUniqueConst(dd,&scaleTerminal1);
    ...
    */
	...
	if (scale > 1) {
	  Terminal scaleTerminal2;
	  scaleTerminal2.set((double) scale);
	  add_scale = cuddUniqueConst(dd,&scaleTerminal2);
	  //add_scale = cuddUniqueConst(dd,(CUDD_VALUE_TYPE)scale);
	    if (add_scale == NULL) {
		Cudd_RecursiveDeref(dd, res);
		return(NULL);
