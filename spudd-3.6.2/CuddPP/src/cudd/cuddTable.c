...
int NewNodeFlag;
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddTable.c,v 2.3 2005/04/14 01:43:52 jhoey Exp $";
#endif
  ...
  DdManager * unique)
{
	...
	if (mem == NULL) {
	  ...
	  ** NULL. */
	  if (cuddGarbageCollect(unique,1) == 0) { /* ==0 means no garbage collecting was done*/
  ...
  unsigned int looseUpTo /* Limit for fast table growth */)
{
    ...
    cuddE(sentinel) = NULL;
    sentinel->next = NULL;
    Terminal *epsilon = new Terminal(DD_EPSILON);
    unique->epsilon = epsilon;
  ...
  DdManager * unique)
{
	...
	//this is seg faulting now
	//and I don't know why because it worked
	//fine for me... sigh.  how impossible
	//and how annoying
	if (node->type.value) {
...
#pragma pointer_size restore
	...
	for (j = 0; j < slots; j++) {
		...
		next = node->next;
		if (node->ref == 0) {
		  //std::cout << "\nDeleted terminal with value " << node->type.value->get_max()
		  //    << " pointer " << node->type.value;
		  //printf("We delete this terminal:%f %f\n",(*node->type.value).get_min(),(*node->type.value).get_max());  
		  deleted++;
		  //We delete the terminal of this dead constant. 
		  //this way I can allocate a new terminal every time I am in cuddUniqueConst
		  delete(node->type.value);
  ...
  CUDD_VALUE_TYPE  value)
{
	...
	//replaces CUDD_MAXINDEX by CUDD_CONST_INDEX Robert Feb06,03
    ...
    //What is this HACK!!!!!
    //I am trying this hack to see if it solve the problem of the zeros.......
    //multiply the max by PI and add the min....this should give me a good unique number
    //split.value = (*value).get_min() + M_PI * (*value).get_max();    
    
    //Apr 4th 2001: We now need to specify a hasCode method for each class of terminal nodes
    //that we are creating.  We have to create unique identifiers for the values that our terminal
    //nodes can have.  The hashCode should be a double cause we are splitting it in the following step.

    split.value = (*value).hashCode();      
    pos = ddHash(split.bits[0], split.bits[1], unique->constants.shift);

    //fprintf(stderr,"pair: %s    hash code:%20.15f    bit1: %d   bit2: %d   pos: %d\n",(*value).toString(),split.value,split.bits[0],split.bits[1],pos);
    ...
    while (looking != NULL) {
      if ((*looking->type.value) == (*value)) {
	    // removed this because its redundant - we already do this check in ==
	    //	|| ddEqualVal((*looking->type.value),(*value),(*unique->epsilon))) {
	  if (looking->ref == 0) {
	    cuddReclaim(unique,looking);
	  }
	  //printf("WE JUST FOUND A MATCHING TERMINAL:%f %f\n",(*looking->type.value).get_min(),(*looking->type.value).get_max());
	  //delete(value);
	  return(looking);
	}
	looking = looking->next;
    ...
    //this might now work........
    //if(NewNodeFlag == 1 || (looking->type.value == NULL)){
    
    Terminal *newTerminal = new Terminal(*value);

    looking->type.value = newTerminal;  
  ...
  int i)
{
	...
	*/
	...
	for (j = 0; (unsigned) j < oldslots; j++) {
	    ...
	    while (node != NULL) {
		next = node->next;
		//split.value = (*cuddV(node)).get_min() + (M_PI * ((*cuddV(node)).get_max()));
		//Add the new hashCode method here as well.  Rob. 4-4-01
		split.value = (*cuddV(node)).hashCode();
