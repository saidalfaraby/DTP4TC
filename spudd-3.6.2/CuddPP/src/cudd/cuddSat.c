...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddSat.c,v 2.0 2003/02/07 00:12:37 staubin Exp $";
#endif
  ...
  int  pr /* verbosity level */)
{
	...
	if (ddEqualVal((*cuddV(f)),(*cuddV(g)),(*tolerance))) {
		...
		//Rob: Apr 6 2001: Might need to change this: book keeping purposes...
		(void) fprintf(dd->out,
			       "f: address = %lx\t value = %40.30f \n",
			       (unsigned long) f, (*cuddV(f)).get_val());
		(void) fprintf(dd->out,
			       "g: address = %lx\t value = %40.30f \n",
			       (unsigned long) g, (*cuddV(g)).get_val());
#else
		(void) fprintf(dd->out,
			       "f: address = %x\t value = %40.30f \n",
			       (unsigned) f,(*cuddV(f)).get_val());
		(void) fprintf(dd->out,
			       "g: address = %x\t value = %40.30f \n",
			       (unsigned) g, (*cuddV(g)).get_val());
#endif
	    }
	    return(0);
	}
