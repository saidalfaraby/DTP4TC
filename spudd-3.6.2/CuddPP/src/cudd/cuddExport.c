...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddExport.c,v 2.0 2003/02/07 00:12:33 staubin Exp $";
#endif
...
//	    (mask & (long) scan) / sizeof(DdNode), (*cuddV(scan)).get_val());
	      char *valstring = (*cuddV(scan)).toString();
  ...
  long  mask)
{
    ...
    if (Cudd_IsConstant(f)) {
      //retval = fprintf(fp, "l(\"%lx\",n(\"constant\",[a(\"OBJECT\",\"%g \")],[]))", id, (*cuddV(f)).get_val());
      char *valstring = (*cuddV(f)).toString();
      retval = fprintf(fp, "l(\"%lx\",n(\"constant\",[a(\"OBJECT\",\"%s \")],[]))", id, valstring);
      free(valstring);
        if (retval == EOF) {
            return(0);
        } else {
