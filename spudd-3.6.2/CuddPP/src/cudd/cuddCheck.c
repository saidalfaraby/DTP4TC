...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddCheck.c,v 2.0 2003/02/07 00:12:32 staubin Exp $";
#endif
  ...
  DdManager * table)
{
	...
	while (f != NULL) {
		...
		if (f->index != CUDD_MAXINDEX) {
		    ...
		    //    (unsigned)f,f->index,f->ref,(*cuddV(f)).get_val());
		    // Apr 6 2001: use the print method..... 

		    fprintf(table->err,
		        "       node 0x%x, id = %d, ref = %d, value = %s \n",
		        (unsigned)f,f->index,f->ref,(*cuddV(f)).toString());   
