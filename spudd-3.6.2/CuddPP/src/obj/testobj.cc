...
static void testTerminal(Cudd& mgr, int verbosity);
...
main()
{
    ...
    //cout<<"WE are in testTERMINAL"<<endl;
    //testTerminal(mgr,verbosity);
...
static void testTerminal(Cudd& mgr,int verbosity)
{
  Terminal newTerminal, newTerminal2,fiveTerminal;

  newTerminal.set_both(5.0,9.0);
  newTerminal2.set_both(2.0,8.0);
  fiveTerminal.set_both(5.0,5.0);
  
  ADD s = mgr.constant(newTerminal);
  ADD t = mgr.constant(newTerminal2);
  cout << "t"; t.print(2,verbosity);
  cout << "s"; s.print(2,verbosity);
  ADD w = t+s;
  cout << "w"; w.print(2,verbosity);

  ADD five = mgr.constant(fiveTerminal);
  ADD x = five * w;
  cout << "x"; x.print(2,verbosity);
  
  ADD max = s.Maximum(t);
  cout << "max"; max.print(2,verbosity);
}
*/

/*
static void
testAdd(Cudd& mgr,int verbosity)
{
  ...
  // variables.
  ADD p = mgr.addVar(0);
  cout << "p"; p.print(2,verbosity);
  
  ADD q = mgr.addVar(1);
  cout << "q"; q.print(2,verbosity);
  ...
  //CUDD_VALUE_TYPE is now TERMINAL
  ...
  cout << "s is" << (s <= p ? "" : " not") << " less than or equal to p\n";
