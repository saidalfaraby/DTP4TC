...
static int DddmpCuddDdArrayStoreCnf(DdManager *ddMgr, DdNode **f, int rootN, Dddmp_DecompCnfStoreType mode, char **varNames, int *bddIds, int *bddAuxIds, int *cnfIds, int idInitial, int edgeInTh, int pathLengthTh, char *fname, FILE *fp, int *varNewNPtr);
  ...
  )
{
  ...
  retValue = Dddmp_cuddBddArrayStoreCnf (ddMgr, tmpArray, 1, mode, varNames,
    bddIds, bddAuxIds, cnfIds, idInitial, edgeInTh, pathLengthTh,
    fname, fp, varNewNPtr);
  ...
  )
{
  ...
  retValue2 = DddmpCuddDdArrayStoreCnf (ddMgr, f, rootN, mode, varNames,
    bddIds, bddAuxIds, cnfIds, idInitial, edgeInTh, pathLengthTh,
    fname, fp, varNewNPtr);
  ...
  )
{
  ...
  int *bddIdsInSupport, *permIdsInSupport, *rootStartLine;
  ...
  retValue = fprintf (fp, "c # BDD stored by the DDDMP tool in CNF format\n");
  ...
  flagVar = 0;
  if (varNames == NULL) {
    fprintf (fp, "c # Warning: null variable names ... create DUMMY names.\n");
    fprintf (fp, "c #\n");
    fflush (stderr);
  ...
  fprintf (fp, "c .rootids");
  for (i=0; i<rootN; i++) {
    Dddmp_Assert (rootStartLine[i]==(-1),
      "Init Line for CNF file = (-1).");
    fprintf (fp, " %5d", rootStartLine[i]);
  }
  fprintf (fp, "\n");
  ...
  for (i=0; i<25; i++) {
    if (i<strlen (tmpString)) {
      retValue = fprintf (fp, "%c", tmpString[i]);
    } else {
      retValue = fprintf (fp, " ");
    }
  }
  ...
  )
{
  ...
  for (i=0; i<rootN; i++) {
    if (f[i] != NULL) {
      ...
      if (retValue == 0) {
        (void) fprintf (stderr, "DdStoreCnf: Error in recursive node store\n");
        fflush (stderr);
      }
