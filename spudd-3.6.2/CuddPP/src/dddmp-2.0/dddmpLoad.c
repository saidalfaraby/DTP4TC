...
static int DddmpCuddDdArrayLoad(Dddmp_DecompType dd_type, DdManager *ddMgr, Dddmp_RootMatchType rootMatchMode, char **rootmatchnames, Dddmp_VarMatchType varMatchMode, char **varmatchnames, int *varmatchauxids, int *varcomposeids, int mode, char *file, FILE *fp, DdNode ***pproots);
  ...
  )
{
  ...
  retValue = DddmpCuddDdArrayLoad(DDDMP_ADD,ddMgr,rootMatchMode,
    rootmatchnames, varMatchMode,varmatchnames,varmatchauxids,varcomposeids,
...
Dddmp_cuddHeaderLoad (
  Dddmp_DecompType *dd_type /* OUT: selects the proper decomp type */,
...
DddmpCuddDdArrayLoad (
  Dddmp_DecompType dd_type          /* IN: Selects decomp type */,
  DdManager *ddMgr                  /* IN: DD Manager */,
  Dddmp_RootMatchType rootMatchMode /* IN: storing mode selector */,
  char **rootmatchnames             /* IN: sorted names for loaded roots */,
  Dddmp_VarMatchType varMatchMode   /* IN: storing mode selector */,
  ...
  )
{
  ...
  *pproots = NULL;
  ...
  if (Hdr->dd_type != dd_type) {
    (void) fprintf (stderr, "DdLoad Error: dd_type mismatch\n");
  ...
  switch (varMatchMode) {
    ...
    case DDDMP_VAR_MATCHPERMIDS:
      for (i=0; i<Hdr->nsuppvars; i++)
        convertids[permsupport[i]] = Cudd_ReadInvPerm(ddMgr,Hdr->permids[i]);
      break;
    ...
    case DDDMP_VAR_MATCHNAMES:
      ...
      /* generate convertids array */
      for (i=0; i<Hdr->nsuppvars; i++) {
        assert (Hdr->suppVarNames[i] != NULL);
  ...
  for (i=1; i<=Hdr->nnodes; i++) {
    ...
    switch (mode) {
      ...
      case DDDMP_MODE_TEXT:
        ...
        switch (Hdr->varinfo) {
          ...
          case DDDMP_VARNAMES:
            retValue = fscanf(fp, "%d %*s",&id);
	    retValue += fscanf(fp,"%d %d", &idT, &idE); 
	    if (idT == 0 && idE == 0) {
	      retValue += addConstant.readString(fp,buf);
	    }else {
	      retValue += fscanf(fp,"%s\n",buf);
	    }
            Dddmp_CheckAndGotoLabel (retValue<4,
              "Error Reading Nodes in Text Mode.", failure);
            break;
          case DDDMP_VARDEFAULT:
            retValue = fscanf(fp, "%d",&id);
	    retValue += fscanf(fp,"%d %d", &idT, &idE); 
	    if (idT == 0 && idE == 0) 
	      retValue += addConstant.readString(fp,buf);
	    else
        ...
        if (idT==0 && idE==0)
        {
          ...
          if (strcmp(buf, "1")==0) {
            ...
            if (strcmp(buf, "0")==0) {
              ...
              //Robert: Feb06, 2003.....update this to work with terminal type
	      addConstant.parseString(buf);
	      //pnodes[i] = Cudd_addConst(ddMgr, (CUDD_VALUE_TYPE)addConstant);       
	      pnodes[i] = Cudd_addConst(ddMgr,&addConstant);
	    }
          }
    ...
    switch (dd_type) {
      case DDDMP_BDD: 
        pnodes[i] = Cudd_bddIte(ddMgr, Cudd_bddIthVar(ddMgr,var), T, E);
        break;
      case DDDMP_ADD: 
        { 
  ...
  Dddmp_CheckAndGotoLabel (!matchkeywd(buf, ".end"), "Error .end not found.",
    failure);
