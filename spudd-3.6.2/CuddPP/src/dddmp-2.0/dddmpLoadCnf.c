  ...
  )
{
  ...
  int i, j, var, **cnfTableLocal;
  int *clause;

  cnfTableLocal = DDDMP_ALLOC (int *, Hdr->nClausesCnf);
  clause = DDDMP_ALLOC (int, 2*Hdr->nVarsCnf);
...
{
  DdNode **rel, *lit, *tmp1, *tmp2, **rootsPtr;
  DdNode *cubeAllVar, *cubeBddVar, *cubeCnfVar;
