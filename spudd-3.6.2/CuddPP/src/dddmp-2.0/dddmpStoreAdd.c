...
DddmpCuddDdArrayStoreBdd (
  Dddmp_DecompType dd_type  /* IN: Selects the decomp type: BDD or ADD */,
  DdManager *ddMgr          /* IN: DD Manager */,
  char *ddname              /* IN: DD name (or NULL) */,
  int nRoots                /* IN: number of output BDD roots to be stored */,
  ...
  )
{
  ...
  Dddmp_CheckAndGotoLabel (dd_type==DDDMP_BDD,
  ...
  )
{
  ...
  if (Cudd_IsConstant(f)) {
    ...
    if (f == Cudd_ReadOne(ddMgr)) {
      if ((varnames != NULL) || (outids != NULL)) {
        retValue = fprintf (fp, "%d T 0 0 1\n", idf);
      } else {
        retValue = fprintf (fp, "%d 0 0 1\n", idf);
      }
    ...
    */
    ...
    if ((varnames != NULL) || (outids != NULL)) {
      ...
      retValue = fprintf (fp, "%d T 0 0 %s\n",idf,(*Cudd_V(f)).toString());
    } else {
      //retValue = fprintf (fp, "%d %g 0 0\n",idf, (*Cudd_V(f)).get_min());
      retValue = fprintf (fp, "%d 0 0 %s\n",idf, (*Cudd_V(f)).toString());
    }
  ...
  if (varnames != NULL) {   
    retValue = fprintf (fp, "%d %s %d %d %d \n",
       idf, varnames[vf], idT, idE, supportids[vf]);
  ...
  if (outids != NULL) {   
    retValue = fprintf (fp, "%d %d %d %d %d\n",
       idf, outids[vf], idT, idE, supportids[vf]);
